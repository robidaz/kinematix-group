...
VendorVault is a **proof of concept** formulated entirely by Zach Robida to demonstrate the ability to leverage AI-assisted development to rapidly create value-added tools — without sacrificing quality. The goal was to show that modern AI tooling can meaningfully compress the gap between idea and working product, while still producing something polished and purposeful. This entire application went from design to deployment in ~7 hours and cost ~$35 including token consumption and domain name.
...
- **Peer experience capture.** Reputation scores give a normalized signal, but they don't convey the texture of lived experience. Team members needed a way to record and surface first-hand accounts — positive and negative — directly on the vendor record. These experiences needed to carry attribution (who said it, in what role) so readers can weigh the context appropriately.
...
### Peer experience and testimonials

Each vendor record carries a `testimonials` array populated with pre-seeded entries sourced from the employee roster. In the grid, a vendor with existing testimonials displays a count badge on the Testimonials column; a vendor with none shows a dashed-circle prompt. Both open the vendor detail dialog. Inside the dialog, the full testimonial list is displayed with each entry attributed to a named employee and their role (Sales or Technical). A "Describe your Experience" button expands an inline form where any team member can select their name from the employee dropdown — their role auto-fills — and submit a plain-text account of up to 250 characters. Submitted testimonials appear immediately in the list within the same session. The design keeps the submission flow lightweight and in-context, minimizing the distance between reading about a vendor and contributing to its record.

### Demo / portfolio fitness

The application has no backend, no authentication, and no database — all data is loaded from local JSON, markdown, and text assets. The Anthropic API call is the only outbound HTTP call the application makes, and only on user-initiated submit.
